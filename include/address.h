...
#ifndef UNP_ADDRESS_H
#define UNP_ADDRESS_H
 ...
 */
    struct Address4 : public sockaddr_in {
        ...
        inline size_t size() const { return sizeof(Address4); }

        //
        // presentation to number and so on
        //
        bool inet_pton(const char *p) { return ::inet_pton(AF_INET, p, (void *) &sin_addr.s_addr); }

        bool inet_pton(std::string &s) { return ::inet_pton(AF_INET, s.c_str(), (void *) &sin_addr.s_addr); }
    ...
    struct AddressUn : public sockaddr_un {
        ...
        size_t size() const { return (long) &sun_path - (long) this + strlen(sun_path); }

        void unlink() const
        { ::unlink(sun_path); }

        bool operator==(const AddressUn &rhs) { return bcmp(this, &rhs, sizeof(AddressUn)) == 0; }

        bool operator==(const sockaddr_un &rhs) { return bcmp(this, &rhs, sizeof(AddressUn)) == 0; }
